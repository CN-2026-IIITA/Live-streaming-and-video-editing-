...
io.on('connection', (socket) => {
  ...
  socket.on('make-reel', () => {
    ...
    if (!fs.existsSync(activeStreamPath)) {
      socket.emit('reel-error', { message: 'No active stream found!' });
      return;
    }
    ...
    ffmpeg(activeStreamPath)
      ...
      .on('end', () => {
        ...
        io.emit('reel-ready', { message: 'New Reel generated successfully!' });
      })
      .on('error', (err) => {
...
server.listen(3000, '0.0.0.0', () => {
  console.log('🚀 Backend running on http://10.132.88.62:3000');
});
